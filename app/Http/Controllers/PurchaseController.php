...
class PurchaseController extends Controller
{
	  ...
	  return ($mod == '')? array($jy, $jm, $jd) : $jy.$mod.$jm.$mod.$jd;
		...
		$j_date_string = gregorian_to_jalali($gy, $gm, $gd, '/'); //خروجی رشته
                ...
                if($purchase->payment_status == 1)
                   ...
                   $iran1=substr($iran, 0, -3)  ;

                $nestedData['grand_total'] = $iran1.' '.config('currency');;
                
                 $iran= number_format($purchase->paid_amount, 2);
                                                
                 $iran2=substr($iran, 0, -3)  ;
                   
                $nestedData['paid_amount'] =$iran2.' '.config('currency');;
                
                $iran=  number_format($purchase->grand_total - $purchase->paid_amount, 2);
                                                
                 $iran3=substr($iran, 0, -3)  ;
                $nestedData['due'] =$iran3.' '.config('currency');;
    ...
    public function importPurchase(Request $request)
    {
        ...
        foreach ($product_data as $key => $product) {
            ...
            $product_purchase->net_unit_cost = $iran1.' '.config('currency');;
            $product_purchase->discount = $discount[$key] * $qty[$key];
            $product_purchase->tax_rate = $tax[$key]['rate'];
            
            
                    $iran= number_format((float)$product_tax, 2, '.', '');
                    $iran3= substr($iran, 0, -3) ;
            $product_purchase->tax =$iran3;
            
            
                     $iran= number_format((float)$total, 2, '.', '');
                    $iran4= substr($iran, 0, -3) ;
            $product_purchase->total =$iran4; 
            ...
            $lims_purchase_data->total_discount += $discount[$key] * $qty[$key];
              ...
              $iran= number_format((float)$product_tax, 2, '.', '');
              $iran5= substr($iran, 0, -3) ;
            $lims_purchase_data->total_tax +=$iran5; 
    ...
    public function update(Request $request, $id)
    {
        ...
        foreach ($product_id as $key => $pro_id) {
            ...
            if($lims_product_data->is_variant) {
                ...
                $product_purchase['variant_id'] = $lims_product_variant_data->variant_id;
                //مقدار را به جدول نوع محصول اضافه کنید
    ...
    public function addPayment(Request $request)
    {
        ...
        return redirect('purchases')->with('message', 'پرداخت با موفقیت ایجاد شد');
    }
    ...
    foreach ($lims_payment_list as $payment) {
            ...
            $date[] =  $payment->created_at->toTimeString();
 ...
 if ($payment->paying_method == "Gift Card") {
                                $paying_method[] = "کارت هدیه";
 

                                 }
                                  if ($payment->paying_method == "Credit Card") {
                                $paying_method[] = "کارت اعتباری";
