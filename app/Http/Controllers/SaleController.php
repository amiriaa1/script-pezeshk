...
class SaleController extends Controller
{
   ...
   $nestedData['grand_total'] =$iran2 .config('currency');
     else
   $nestedData['grand_total'] =$iran2 .config('currency');
   ...
   $iran3=substr($iran, 0, -3)  ;
    ...
    if(config('currency_position') == 'prefix')
   $nestedData['paid_amount'] = $iran3 .config('currency');
     else
    $nestedData['paid_amount'] = $iran3 .config('currency');
                                   ...
                                   $iran4=substr($iran, 0, -3)  ;

                $nestedData['due'] = $iran4 .config('currency');
    ...
    public function store(Request $request)
    {
        ...
        if($data['sale_status'] == 3)
            $message = 'فروش با موفقیت به پیش نویس اضافه شد';
        else
        ...
        if($mail_data['email'] && $data['sale_status'] == 1) {
            try {
                ...
                $message = 'فروش با موفقیت انجام گردید ';
            }
        }
    ...
    public function paypalSuccess(Request $request)
    {
        ...
        return redirect('sales')->with('message', 'فروش با موفقیت ایجاد شد');
    }
    ...
    public function paypalPaymentSuccess(Request $request, $id)
    {
        ...
        $data['transaction_id'] = $response['PAYMENTINFO_0_TRANSACTIONID'];
        PaymentWithPaypal::create($data);
        return redirect('sales')->with('message', 'پرداخت با موفقیت ایجاد شد');
    }
    ...
    public function importSale(Request $request)
    {
        ...
        if($ext != 'csv')
            return redirect()->back()->with('message', 'لطفاً یک پرونده CSV بارگذاری کنید');
        ...
        while (!feof($file_handle) ) {
            ...
            if($current_line && $i > 0){
                ...
                if(!$product_data[$i-1])
                    return redirect()->back()->with('message', 'محصول وجود ندارد!');
                $unit[] = Unit::where('unit_code', $current_line[2])->first();
                if(!$unit[$i-1] && $current_line[2] == 'n/a')
                    $unit[$i-1] = 'n/a';
                elseif(!$unit[$i-1]){
                    return redirect()->back()->with('message', 'واحد فروش وجود ندارد!');
                }
                if(strtolower($current_line[5]) != "no tax"){
                    $tax[] = Tax::where('name', $current_line[5])->first();
                    if(!$tax[$i-1])
                        return redirect()->back()->with('message', 'نام مالیات وجود ندارد!');
                }
                else
                    $tax[$i-1]['rate'] = 0;
        ...
        foreach ($product_data as $key => $product) {
            ...
            $product_sale->sale_unit_id = $sale_unit_id;
                           ...
                           $iran= number_format($net_unit_price, 2);

                           $iran2=substr($iran, 0, -3)  ;

            $product_sale->net_unit_price = $iran2;
            $product_sale->discount = $discount[$key] * $qty[$key];
            $product_sale->tax_rate = $tax[$key]['rate'];



                           $iran= number_format($product_tax, 2);
                           $iran2=substr($iran, 0, -3)  ;

            $product_sale->tax = $iran2;


                           $iran= number_format($total, 2);
                          $iran2=substr($iran, 0, -3)  ;

            $product_sale->total = $mail_data['total'][$key] = $iran2;
            ...
            $lims_sale_data->total_tax += $iran2;
            ...
            $lims_sale_data->total_price += $iran2;
        }
        ...
        if($lims_customer_data->email){
            ...
            if($mail_data['email']){
                try{
                    ...
                    $message = 'فروش با موفقیت وارد شد';
                }
            }
        }
        return redirect('sales')->with('message', $message);
    ...
    public function update(Request $request, $id)
    {
        ...
        $lims_customer_data = Customer::find($data['customer_id']);
        $message = 'فروش با موفقیت به روز شد';
        //collecting mail data
        if($lims_customer_data->email){
            $mail_data['email'] = $lims_customer_data->email;
            $mail_data['reference_no'] = $lims_sale_data->reference_no;
    ...
    public function updatePayment(Request $request)
    {
        ...
        if($lims_customer_data->email){
            ...
            try{
                ...
                $message = 'پرداخت با موفقیت به روز شد. ';
            }
        }
        return redirect('sales')->with('message', $message);
    }
    ...
    public function deleteBySelection(Request $request)
    {
        ...
        foreach ($sale_id as $id) {
            ...
            if($lims_sale_data->sale_status == 3)
                $message = 'پیش نویس با موفقیت حذف شد';
            else
